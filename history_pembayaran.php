...
// Pastikan $conn sudah didefinisikan dari koneksi.php
if (!$conn) {
    die("Koneksi database tidak berhasil. Periksa pengaturan koneksi.");
}

// Query untuk mengambil data berdasarkan ID
$dataKasbon = mysqli_query($conn, "SELECT * FROM pembayaran 
                        INNER JOIN pelanggan ON pembayaran.pelanggan_id=pelanggan.id
                        WHERE pembayaran.id='$id'");

$dataKasbon = mysqli_fetch_assoc($dataKasbon); // Mengambil data sebagai array asosiatif
    ...
    <div class="text-overlay">
        <div class="container-row1">
            <div class="container-nama-nilai">
                <div class="text-nama"><?= $dataKasbon['nama_pelanggan'] ?></div>
                <div class="text-nilai"><?= numberFormat($dataKasbon['nominal_pembayaran']) ?></div>
            </div>
            <div class="container-tglpengajuan-tglacc">
                <div class="text-tglpengajuan"><?= $dataKasbon['terbilang'] ?></div>
                <div class="text-tglacc"><?= $dataKasbon['tanggal_bayar'] ?></div>
            </div>
        </div>
        <div class="container-row1-1">
            <div class="title-persetujuan">Disetujui Rp.<span>:</span></div>
            <?php
            $number = $dataKasbon['nominal_bayar'];
            ?>
